...
export default function Contact() {
  ...
  return (
    <Section id="kontakt" className="scroll-mt-12 lg:py-10">
      <SectionHeading subtitle="Wypełnij formularz, a odezwiemy się do Ciebie w ciągu 24 godzin. Możesz też zadzwonić bezpośrednio." className="lg:mb-6">
        Kontakt
      </SectionHeading>

      <div className="mx-auto max-w-7xl flex flex-col-reverse lg:grid lg:grid-cols-2 gap-6 lg:gap-10 items-start">
        
        {/* LEWA KOLUMNA — FAQ + INFO */}
        <AnimateOnScroll className="space-y-4">

            <div className="mb-2 mt-1">
              <h3 className="text-xl font-bold text-dark lg:text-2xl">Najczęściej zadawane pytania</h3>
            </div>
            ...
            {FAQ_CONTACT.map((item, index) => (
              <div key={index} className={`overflow-hidden rounded-2xl border bg-white transition-colors duration-300 ${openFaq === index ? "border-accent/20" : "border-slate-200"}`}>
                <button
                  ...
                  className="flex w-full items-center justify-between px-5 py-3 text-left lg:px-4 lg:py-3"
                >
                  <span className="font-semibold text-dark text-sm lg:text-base">{item.question}</span>
                ...
                <div className={`grid transition-all duration-300 ${openFaq === index ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                  <div className="overflow-hidden">
                    <p className="px-5 pb-3 text-dark/70 text-sm leading-relaxed lg:px-4">{item.answer}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <AnimateOnScroll delay={0.2} className="w-full">
          <form onSubmit={handleSubmit} className="w-full space-y-3 rounded-3xl bg-white p-6 shadow-2xl border border-dark/5 border-t-4 border-t-accent">
            <div className="mb-2">
            ...
            <div>
              ...
              <textarea id="message" name="message" required rows={2} className={`${inputClass} resize-none`} placeholder="Opisz swoje potrzeby..." />
